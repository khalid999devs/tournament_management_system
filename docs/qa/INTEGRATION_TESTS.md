...
The integration suite runs the real registration, review, event-settings and operator-scope services against PostgreSQL. It covers the PRD's P0 integration cases: two students competing for the last place, approve/reject consistency, duplicate transaction IDs after normalization, and each operator assignment scope. Phase 4 adds the match tests: bracket generation, concurrent score events, idempotent retries, stale-version rejection, progression through byes and walkovers, reopen and correction, operator scoring rights, and scoring-rule locks. Phase 5 adds problem reports (scope and capability checks, one report per resent request, resolve once), reminders (one per confirmed player even when runs overlap, cancelled registrations skipped, stuck messages recovered), exports that contain every filtered row, dashboard figures, and the exact content of live-update signals.

...
| Email failures do not roll back database state and can be retried.                 | External delivery runs after commit. Provider failures update the notification to `FAILED`; registration state remains unchanged. `/admin/notifications` exposes delivery status, attempts, safe error text, and retry for failed/queued messages. Resend idempotency keys prevent duplicate provider sends. |

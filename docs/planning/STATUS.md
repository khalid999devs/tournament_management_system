...
| 2026-09-20 | Every complicated panel carries its own illustrated guide, in a modal.                                    | The platform is new to everyone using it, and a diagram answers "what will this button do to my event" better than a paragraph.               |
| 2026-09-20 | Players are searched on demand and chosen one row per person, not per game entry.                         | An event can hold hundreds of entrants; shipping them all with the page was slow, and nobody thinks of a person as three separate entries.    |
| 2026-09-20 | Time, table, postpone and resume are editable from the match list; reopen and cancel are not.             | The first four are everyday corrections. The last two change what operators see and need a reason on the record, so they stay one click away. |
| 2026-09-20 | Layout is checked by measuring the running page, not by reading screenshots.                              | Two rounds of "fixed" were reported from a stale server and a buggy checker. Numbers from the live DOM do not flatter.                        |
